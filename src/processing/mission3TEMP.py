...
def _startup(nav: dict, mis: dict, target_lat: float, target_lon: float):
...
def main():
    ...
    print("[mission3] Waiting for target_location.valid …")
    while True:
        ...
        if tgt.get("valid", False):
            ...
            print(f"[mission3] target_location valid: ({target_lat:.6f}, {target_lon:.6f})")
            break

    mis, nav = _startup(nav, load_state(), target_lat, target_lon)
